...
import { CalendarDays, Home, MessageCircle, Plus, Smile, User } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur md:px-6">
      ...
      <div className="flex items-center gap-2">
        <Button onClick={() => router.push("/dashboard?addMemory=1")} className="h-10 rounded-full bg-gradient-to-r from-[#FF6EC7] to-[#8B5CF6] text-white">
          <Plus className="mr-1 h-4 w-4" /> Add Memory
        </Button>
        <Button variant="ghost" size="icon" onClick={() => router.push("/dashboard/chat")} className="rounded-full">
